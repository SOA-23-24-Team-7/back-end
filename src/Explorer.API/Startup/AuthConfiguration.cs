...
public static class AuthConfiguration
{
    ...
    private static void ConfigureAuthorizationPolicies(IServiceCollection services)
    {
        services.AddAuthorization(options =>
        {
            ...
            options.AddPolicy("authorOrTouristPolicy", policy => policy.RequireAssertion(context => 
            (new[] { "author", "tourist" }).Contains(context.User.Claims.First(c => c.Type == ClaimTypes.Role).Value)));
        });
    }
}